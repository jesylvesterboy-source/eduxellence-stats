...
def ok_ext(n): return "." in n and n.rsplit(".",1)[1].lower() in ALLOWED
...
def classify(df):
    ...
    for c in df.columns:
        ...
        is_num=num.notna().sum()>len(df)*.5 and df[c].nunique()>5
        out.append({"name":c,"dtype":"numeric" if is_num else "categorical",
            "n_unique":int(df[c].nunique()),"missing":int(df[c].isna().sum()),
...
def to_excel(results,analysis_type):
    ...
    with pd.ExcelWriter(buf,engine="openpyxl") as w:
        # Summary
        meta={"Analysis":results.get("test",""),"Generated":datetime.now().strftime("%Y-%m-%d %H:%M"),
              "Website":"https://eduxellence.org","p-value":results.get("p_display","—"),
              "Significance":results.get("significance","—"),"APA Citation":results.get("apa_citation","—")}
        ...
        for key,sheet in[("numeric_summary","Descriptive"),("summary_table","Group Summary"),
            ...
            if results.get(key): pd.DataFrame(results[key]).to_excel(w,sheet_name=sheet,index=False)
...
def index():
    ...
    if p.exists(): return p.read_text(encoding="utf-8"),200,{"Content-Type":"text/html"}
...
def upload_page():
    ...
    if p.exists(): return p.read_text(encoding="utf-8"),200,{"Content-Type":"text/html"}
    from flask import redirect; return redirect("/stats")
...
def upload():
    ...
    if not ok_ext(f.filename): return jsonify({"error":"Use .csv, .xlsx or .xls"}),400
    ...
    try:
        ...
        meta_path=path+".meta"
        with open(meta_path,"w") as mf: json.dump({"path":path},mf)
        recs=recommend_tests(df,cols)
        return jsonify({"session_id":session_id,"meta_path":meta_path,"filename":f.filename,
            "rows":int(len(df)),"cols":int(len(df.columns)),"columns":cols,"preview":prev,"recommendations":recs})
...
def assumptions():
    ...
    meta_path=data.get("meta_path"); analysis_type=data.get("analysis_type"); params=data.get("params",{})
    if not meta_path or not os.path.exists(meta_path): return jsonify({"error":"Session expired."}),400
    try:
...
def transform():
    data=request.get_json()
    meta_path=data.get("meta_path"); column=data.get("column"); transform_type=data.get("transform","log")
    if not meta_path or not os.path.exists(meta_path): return jsonify({"error":"Session expired."}),400
    try:
        with open(meta_path) as mf: meta=json.load(mf)
        df=load_df(meta["path"])
        if transform_type=="log": df_new,new_col=apply_log_transform(df,column)
        ...
        with open(new_meta,"w") as mf: json.dump({"path":new_path},mf)
        return jsonify({"meta_path":new_meta,"new_column":new_col,
            "message":f"Log transform applied. New column '{new_col}' added.","columns":classify(df_new)})
    except Exception as e:
        return jsonify({"error":str(e)}),500
...
def analyze():
    data=request.get_json()
    meta_path=data.get("meta_path"); analysis_type=data.get("analysis_type"); params=data.get("params",{})
    if not meta_path or not os.path.exists(meta_path): return jsonify({"error":"Session expired. Re-upload your file."}),400
    if analysis_type not in ANALYSIS_LABELS: return jsonify({"error":f"Unknown analysis."}),400
    try:
        with open(meta_path) as mf: meta=json.load(mf)
        df=load_df(meta["path"])
    except Exception as e: return jsonify({"error":f"Cannot load file: {e}"}),500
    try: results=run_analysis(df,analysis_type,params)
    except Exception as e: return jsonify({"error":str(e),"detail":traceback.format_exc()}),500
    if "error" in results: return jsonify(results),422
...
def export():
    ...
    try:
        with open(meta_path) as mf: meta=json.load(mf)
        df=load_df(meta["path"]); results=run_analysis(df,analysis_type,params)
        xbytes=to_excel(results,analysis_type)
        fname=f"eduxellence_{analysis_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        return send_file(io.BytesIO(xbytes),as_attachment=True,download_name=fname,
